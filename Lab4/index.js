 ...
 * Завдання 3: теоретична оцінка складності — текст українською нижче та у виводі.
 * Усі коментарі та виводи — українською.
 */
...
// ——— Завдання 3: теоретична оцінка складності (українською) ———
const THEORETICAL_COMPLEXITY = `
ЗАВДАННЯ 3 — ТЕОРЕТИЧНА ОЦІНКА СКЛАДНОСТІ АЛГОРИТМУ СИМУЛЯЦІЇ
================================================================

Теоретична складність алгоритму:

Операції на черзі подій (відсортований масив)
  Додавання події: пошук місця за часом + splice(i, 0, event) — O(M).
  Виймання найближчої за часом: shift() — O(1) амортизовано,
  де M — поточна кількість подій у черзі.

Кількість подій в очікуванні:
  У простих мережах M = O(N + Q_total), але типово M обмежено числом
  відкладених подій; у цій реалізації M найчастіше значно менше E (не одночасно
  всі E подій у черзі), і залежить від завантаження системи. При жорсткій
  навантаженості або коли багато подій плануються одночасно, M може зростати.
  У граничній теоретичній оцінці M = O(E) (якщо всі події накопичилися),
  але практично M = O(N + кількість активних робіт).

Час обробки однієї події:
  O(M) для додавання в чергу подій + O(1) для виймання + O(1) для оновлень
  черги сервера та логіки елементів + O(1) для генерації випадкових проміжків.

Пам'ять:
  Залежить від кількості запланованих подій одночасно: O(M). В додаток —
  O(N) для станів серверів та черг елементів.

Висновок (теорія):
  Середня складність: O(E · M_avg), зазвичай M_avg = O(N), тобто порядку O(E · N).
  Гірший випадок: O(E · M) при M ~ E, тобто O(E²).
`;
...
console.log(THEORETICAL_COMPLEXITY);
